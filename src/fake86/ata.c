...
void
bufwrite8(uint16_t bufpos, uint8_t value)
{
  databuf[bufpos] = value;
}

void
bufwrite16(uint16_t bufpos, uint16_t value)
{
...
void
flipstring(uint8_t* dest, uint8_t* src)
{
  uint16_t i;
  uint8_t cc;
  strcpy(dest, src);
...
void
outATA(uint16_t portnum, uint8_t value)
{
